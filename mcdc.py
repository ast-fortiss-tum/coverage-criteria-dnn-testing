...
class MCDC:
    ...
    def get_normalized_coverage(self):
        ss_coverage = round(100 * len(self.coveredsetss) / len(self.totalset), 2)
        sv_coverage = round(100 * len(self.coveredsetsv) / len(self.totalset), 2)
        vs_coverage = round(100 * len(self.coveredsetvs) / len(self.totalset), 2)
        vv_coverage = round(100 * len(self.coveredsetvv) / len(self.totalset), 2)
        return ss_coverage, sv_coverage, vs_coverage, vv_coverage
...
def coverage_report_mcdc(model, x_train, x_test, trainlimit, testlimit):
    mcdc_coverage = MCDC(model, x_train[:trainlimit], x_test[:testlimit], 0, trainlimit, testlimit)
    mcdc_coverage.opt_coverage(True, True, True, True)
    return mcdc_coverage.get_normalized_coverage()

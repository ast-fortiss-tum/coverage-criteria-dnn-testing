...
def dry_run(indir, fetch_function, coverage_function, queue, f, nccsv):
    ...
    f.write("\nTotal number of tests in test set: " + str(len(test_list)))
    print(f"Total number of tests in test set: {len(test_list)}")
    nccsv.writerow("\n" + str(len(test_list)) + "\n")
    print("\nCOVERAGE REPORT:")
    f.write("\n\nCOVERAGES:")

    for test_name in tqdm(test_list):
        if counter % 10 == 0 and counter < len(test_list):
        ...
        path = indir +"/"+ test_name
        img = np.load(path)

        if img.ndim == 2:  # If the image is (28, 28)
            img = img.reshape(1, 28, 28, 1)
        elif img.ndim == 3 and img.shape[0] == 28 and img.shape[1] == 28:  # If the image is (28, 28, 1)
            img = img.reshape(1, 28, 28, 1)
        elif img.ndim == 4 and img.shape[0] == 2:  # If the image is (2, 28, 28, 1)
    ...
    coverage_values= [queue.dry_run_cov[0],queue.dry_run_cov[1], queue.dry_run_cov[2] ,queue.dry_run_cov[3] , queue.dry_run_cov[4]]
    return coverage_values


def save_coverage_values_to_csv(coverage_values, csv_path):
    with open(csv_path, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Test Suite', 'kMNC', 'TKNC', 'NBC', 'SNAC', 'NC'])
        for i, coverage in enumerate(coverage_values):
            writer.writerow([f'Test Suite {i + 1}'] + list(coverage))

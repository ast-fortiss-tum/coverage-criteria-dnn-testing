...
def add_noise_and_blur(x_batch, noise_level=0.4, blur_sigma=1.7):
...
def test_generate(dataset_name, output_path, num_suites, tests_per_suite, suite_limit, noise_and_blur):
    ...
    return all_test_suites, all_label_suites, model, accuracies, x_train, y_train

def retrain_and_evaluate(all_test_suites, all_label_suites, original_accuracies, model, output_path, x_train, y_train):
    improvements = []
    original_avg_accuracy = np.mean(original_accuracies)

    for i in range(len(all_test_suites)):
        # Reload the original model to ensure fresh start for each retraining
        model = load_model('lenet5.h5')
        model.compile(optimizer=Adam(), loss='categorical_crossentropy', metrics=['accuracy'])

        # Load failed tests for current suite
        failed_tests, failed_labels = load_failed_tests(os.path.join(output_path, 'failed_tests'), 1)
        ...
        if len(failed_tests) > 0:
            ...
            print(f"Retraining model with failed tests from Test Suite {i + 1}")
            model.fit(x_train_combined, y_train_combined, epochs=5, batch_size=32)
            retrained_model_path = os.path.join(output_path, f'retrained_model_suite_{i + 1}.h5')
        ...
        for j in range(len(all_test_suites)):
            if j != i:
                accuracy, _ = calculate_accuracies([all_test_suites[j]], [all_label_suites[j]], retrained_model)
                remaining_accuracies.append(accuracy[0])

        # Calculate the average accuracy of the remaining test suites
        average_remaining_accuracy = np.mean(remaining_accuracies)

        # Calculate the improvement
        improvement = average_remaining_accuracy - original_avg_accuracy
        improvements.append(improvement)

        print(f"Test Suite {i + 1}: Average accuracy improvement is {improvement:.2f}")

    # Save the improvements to a CSV file
    with open(os.path.join(output_path, 'accuracy_improvements.csv'), 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Test Suite', 'Accuracy Improvement'])
        for i, improvement in enumerate(improvements):
            writer.writerow([f'Test Suite {i + 1}', improvement])
...
# Generate test suites and calculate original accuracies
all_test_suites, all_label_suites, model, original_accuracies, x_train, y_train = test_generate(dataset_name, 'test_suites_imp', num_suites, tests_per_suite, suite_limit, noise_and_blur)

# Retrain and evaluate the model using each test suite's failed tests
retrain_and_evaluate(all_test_suites, all_label_suites, original_accuracies, model, 'test_suites_imp', x_train, y_train)

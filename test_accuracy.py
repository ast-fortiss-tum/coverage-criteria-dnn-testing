...
def recalculate_remaining_accuracies(test_suites, label_suites, retrained_model, start_index, output_path):
    ...
    save_accuracies(accuracies, os.path.join(output_path, 'retrained_model_accuracies.csv'))
...
def test_generate(dataset_name, output_path, num_suites, tests_per_suite, suite_limit,noise_and_blur):
    ...
    # Retrain the model with the failed tests
    failed_tests, failed_labels = load_failed_tests(os.path.join(output_path, 'failed_tests'), suite_limit)
    if len(failed_tests) > 0:
        print(f"Retraining model with {len(failed_tests)} failed tests.")
        model.fit(failed_tests, failed_labels, epochs=5, batch_size=32)
...
num_suites = 90  # Number of test suites
tests_per_suite = 100  # Number of tests per suite
suite_limit = 3  # Limit for the number of test suites to save failed tests
noise_and_blur= True
noise_level = 0.3  # Adjust to increase/decrease noise 0.4 0.8 1
blur_sigma = 1.3  # Adjust to increase/decrease blur 1.7 2 2.2
